...
export class TransactionFetcher {
	...
	async fetchTransactions(
		...
		const lookbackDays =
			options.lookbackDays || config.getTransactionLookbackDays();
		const limit = options.limit || 1000;

		// Calculate the timestamp for lookback period
		const now = Date.now();
		const lookbackTimestamp =
			Math.floor(now / 1000) - lookbackDays * 24 * 60 * 60;
	...
	private async fetchTransactionDetails(
		...
		for (const sig of signatures) {
			try {
				...
				console.error(
					`Failed to parse transaction ${sig.signature}:`,
					error
				);
	...
	private async parseTransaction(
		...
		try {
			...
			const details = this.extractTransactionDetails(
				walletPublicKey,
				tx,
				signatureInfo
			);
			...
			console.error(
				`Error parsing transaction ${signatureInfo.signature}:`,
				error
			);
			return null;
		}
		...
		if (tx.meta && tx.transaction.message.accountKeys) {
			...
			if (walletIndex !== -1) {
				...
				if (balanceChange < -details.fee) {
					details.type = "sent";
					details.amount =
						Math.abs(balanceChange + details.fee) / 1e9; // Convert lamports to SOL
